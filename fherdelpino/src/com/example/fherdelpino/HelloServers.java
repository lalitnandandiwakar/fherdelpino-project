...
public class HelloServers extends HttpServlet {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -8320655607666484392L;
	 ...
	 * @param writer
	 *            The writer to use when printing messages.
	 */
	...
	private void handleOperations(HttpServletRequest req,
		...
		if (grow != null) {
			try {
				...
				res.getWriter().append(
						"The grow parameter must have an integer value.");
			}
		}
		if (State.size() > 0) {
		...
		if (loop != null) {
			try {
				...
				res.getWriter().append(
						"The loop parameter must have an integer value.");
			}
		}
		...
		}
		{
			if (req.getCookies() != null) {
				...
				for (Cookie c : req.getCookies()) {
					writer.append(c.getName() + "(" + c.getDomain() + ": "
